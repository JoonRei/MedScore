...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: scoreRows } = ids.length
    ? await db.from("scores").select("assessment_id,score,result_status").eq("student_id", student.id).in("assessment_id", ids)
    : { data: [] as any[] };
    ...
    <div className="result-list">
      {(assessments || []).map((assessment: any) => {
        ...
        return <div className={`result-card ${absent ? "is-absent" : ""}`} key={assessment.id}>
          ...
          <div className="result-score">{absent ? <><strong>Did not take</strong><small>No score recorded</small></> : <><strong>{score === undefined ? "—" : `${score} / ${assessment.total_score}`}</strong><small>{score === undefined ? "Not recorded" : formatPercent(percent(Number(score), Number(assessment.total_score)))}</small></>}</div>
        </div>;
      })}
