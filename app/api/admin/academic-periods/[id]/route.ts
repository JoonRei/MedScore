...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data: target, error: lookupError } = await db.from("academic_periods").select("id").eq("id", id).maybeSingle();
    if (lookupError) throw lookupError;
    if (!target) return NextResponse.json({ error: "Academic period not found." }, { status: 404 });

    const { error: clearError } = await db.from("academic_periods").update({ is_active: false }).eq("is_active", true);
    if (clearError) throw clearError;
    const { error } = await db.from("academic_periods").update({ is_active: true }).eq("id", id);
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const db = createAdminClient();
    const { data: period, error: lookupError } = await db.from("academic_periods").select("id,academic_year,term,is_active").eq("id", id).maybeSingle();
    if (lookupError) throw lookupError;
    ...
    const { count, error: subjectError } = await db.from("subjects").select("id", { count: "exact", head: true }).eq("academic_year", period.academic_year).eq("term", period.term);
    if (subjectError) throw subjectError;
    if (count) return NextResponse.json({ error: "This period is already used by subjects and cannot be removed." }, { status: 400 });

    const { error } = await db.from("academic_periods").delete().eq("id", id);
    if (error) throw error;
    return NextResponse.json({ ok: true });
  } catch {
