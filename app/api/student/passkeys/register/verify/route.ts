...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!challengeId || !response?.id) return NextResponse.json({ error: "Fingerprint setup response is incomplete." }, { status: 400 });
    ...
    if (!challenge || new Date(challenge.expires_at).getTime() < Date.now()) return NextResponse.json({ error: "Fingerprint setup expired. Try again." }, { status: 400 });
    ...
    if (!verification.verified || !verification.registrationInfo) return NextResponse.json({ error: "Fingerprint verification was not completed." }, { status: 400 });
    ...
    return NextResponse.json({ error: "Unable to enable fingerprint sign-in." }, { status: 400 });
  }
}
