...
export function StudentsClient({ students, subjects }: { students: StudentRow[]; subjects: SubjectOption[] }) {
  ...
  async function submitStudent(event: React.FormEvent<HTMLFormElement>, student?: StudentRow) {
    ...
    const payload = {
      ...
      codeName: form.get("codeName"), pin: form.get("pin"), yearLevel: form.get("yearLevel"), subjectIds: form.getAll("subjectIds"),
    };
  ...
  return (
    <>
      ...
      {error && !showAdd && !editStudent && !resetStudent && !deleteStudent && <div className="alert alert-error page-feedback">{error}<button type="button" onClick={() => setError("")}>Dismiss</button></div>}

      <div className="panel data-panel">
        <div className="panel-header panel-header-stack-mobile">
          <div><h2>Student accounts</h2><p>Manage student identity, access and subject enrollment from one place.</p></div>
      ...
      {showAdd && (
        <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && closeModals()}>
          <form className="modal modal-wide" onSubmit={(event) => submitStudent(event)}>
            <div className="modal-header"><div><span className="modal-eyebrow">Student access</span><h2>Add student</h2><p>Create an account and assign current subjects.</p></div><button className="modal-close" type="button" onClick={closeModals} aria-label="Close"><CloseIcon size={20} /></button></div>
      ...
      <ConfirmDialog
        ...
        description={deleteStudent ? `This permanently removes ${deleteStudent.first_name} ${deleteStudent.last_name}, including their enrollments, recorded scores and active sessions. This cannot be undone.` : ""}
