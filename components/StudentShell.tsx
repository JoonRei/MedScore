...
import { BookIcon, ChartIcon, HomeIcon, LogoutIcon, NotificationIcon, SettingsIcon } from "@/components/icons";
...
const nav = [
  ...
  { href: "/student/notifications", label: "Notifications", mobileLabel: "Notifications", icon: NotificationIcon, notifications: true },
        ...
        <aside className="sidebar student-sidebar">
          ...
          <nav className="sidebar-nav" aria-label="Student navigation">
            {nav.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} prefetch={true} className={cn("nav-item student-nav-link", active && "active")}>
                  <span className="student-nav-icon-wrap"><Icon size={19}/>{item.notifications && <StudentNotificationBadge className="is-nav" />}</span>
