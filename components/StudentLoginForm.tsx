...
export function StudentLoginForm() {
  ...
  async function deviceSignIn() {
    ...
    try {
      ...
      if (!optionsResponse.ok) { setDeviceEnabled(false); setError(optionsBody.error || "Device sign-in is not available."); return; }
      ...
      if (!verifyResponse.ok || !verifyBody.verified) { setError(verifyBody.error || "Device sign-in failed."); return; }
      finishLogin();
    } catch (err) {
      const name = err instanceof Error ? err.name : "";
      setError(name === "NotAllowedError" ? "Device verification was cancelled." : "Unable to use device sign-in right now.");
    } finally { setDeviceLoading(false); }
  }

  const deviceButtonDisabled = loading || deviceLoading || checkingDevice || !deviceEnabled;
  ...
  return <form className="login-form" onSubmit={submit} autoComplete="off" data-form-type="other">
    ...
    {deviceSupported && <div className="login-device-option"><span>or</span><button className="button button-secondary button-block" type="button" disabled={deviceButtonDisabled} onClick={() => void deviceSignIn()}>{deviceLoading ? "Verifying device…" : checkingDevice ? "Checking device sign-in…" : "Use fingerprint / Face ID"}</button>{!deviceEnabled && !checkingDevice && <small>Available only when device sign-in is enabled for this code name.</small>}</div>}
  </form>;
}
